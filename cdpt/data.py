...
class DataCollate:
    ...
    def __call__(self, data):
        ...
        xupos_seq_tensor = xupos_seq_tensor[word_perm_idx]
        ...
        if self.parser._edims > 0:
            ...
            emb_tensor = np.zeros((batch_size, max_seq_len, self.parser._edims), dtype=np.float32)
            for i in range(batch_size):
                for j, n in enumerate(norms[i]):
                    if n in self.parser._external_mappings:
                        emb_tensor[i, j] = self.parser._external_embeddings[self.parser._external_mappings[n]]
